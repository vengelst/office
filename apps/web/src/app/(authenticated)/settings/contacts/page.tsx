...
export default function ContactsSettingsPage(): React.ReactNode {
  const t = texts.settings.contacts;
  ...
  const handleSave = async (): Promise<void> => {
    setSaving(true);
    try {
      await settingsApi.saveContactsConfig({ enabled });
  ...
  const handleTest = async (): Promise<void> => {
    setTesting(true);
    try {
      const result = await settingsApi.testContactsConnection();
      if (result.success) {
