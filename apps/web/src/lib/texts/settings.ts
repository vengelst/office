/**
 * UI-Texte für Domäne `settings` (DE-Labels der Office-Web-App).
 * Nur String-Konstanten – absichtlich ohne Kommentar je Key.
 */

export const settings = {
    title: 'Einstellungen',
    placeholder: 'Einstellungen folgen in einem späteren Auftrag.',
    email: {
      title: 'E-Mail-Konfiguration',
      subtitle: 'SMTP-Einstellungen für den E-Mail-Versand',
      host: 'SMTP-Host',
      port: 'SMTP-Port',
      user: 'SMTP-Benutzer',
      pass: 'SMTP-Passwort',
      fromName: 'Absender-Name',
      fromEmail: 'Absender-E-Mail',
      secure: 'TLS/SSL verwenden',
      testEmail: 'Test-E-Mail senden',
      testEmailTo: 'Empfänger für Test',
      testEmailPlaceholder: 'test@example.com',
      save: 'Speichern',
      saving: 'Wird gespeichert …',
      sending: 'Wird gesendet …',
      toast: {
        saved: 'SMTP-Konfiguration gespeichert.',
        testSuccess: 'Test-E-Mail erfolgreich gesendet.',
        testFailed: 'Test-E-Mail fehlgeschlagen.',
        error: 'Aktion fehlgeschlagen.',
      },
    },
    storage: {
      title: 'Speicher & Cloud',
      subtitle: 'Google Drive Synchronisation konfigurieren',
      enabled: 'Google Drive aktivieren',
      folderId: 'Google Drive Ordner-ID',
      folderIdPlaceholder: 'z.B. 1aBcDeFgHiJkLmNoPqRsTuVwXyZ',
      serviceAccountJson: 'Service Account JSON Key',
      serviceAccountPlaceholder: 'JSON-Inhalt hier einfügen …',
      testConnection: 'Verbindung testen',
      testing: 'Wird getestet …',
      info: 'Dokumente werden parallel in MinIO (lokal) und Google Drive gespeichert.',
      save: 'Speichern',
      saving: 'Wird gespeichert …',
      toast: {
        saved: 'Speicher-Konfiguration gespeichert.',
        testSuccess: 'Verbindung erfolgreich.',
        testFailed: 'Verbindungstest fehlgeschlagen.',
        error: 'Aktion fehlgeschlagen.',
      },
    },
    contacts: {
      title: 'Google Contacts',
      subtitle:
        'Ansprechpartner aus Office in Google Kontakte synchronisieren',
      enabled: 'Google Contacts Sync aktivieren',
      credentialsOk: 'Service Account und Impersonation sind unter Speicher & Cloud hinterlegt.',
      credentialsMissing:
        'Zuerst unter Speicher & Cloud Service Account JSON und Impersonation-E-Mail hinterlegen.',
      openStorage: 'Zu Speicher & Cloud',
      impersonate: 'Kontakte landen im Google-Konto von',
      setupTitle: 'Google-Admin (einmalig)',
      setupPeopleApi:
        'In der Google Cloud Console die People API für das Projekt aktivieren.',
      setupDwd:
        'In der Google Admin Console Domain-Wide Delegation: Scope https://www.googleapis.com/auth/contacts für denselben Service Account ergänzen (zusätzlich zum Drive-Scope).',
      testConnection: 'Verbindung testen',
      testing: 'Wird getestet …',
      save: 'Speichern',
      saving: 'Wird gespeichert …',
      toast: {
        saved: 'Contacts-Konfiguration gespeichert.',
        testSuccess: 'People API Verbindung erfolgreich.',
        testFailed: 'Verbindungstest fehlgeschlagen.',
        error: 'Aktion fehlgeschlagen.',
      },
    },
    calendar: {
      title: 'Google Calendar',
      subtitle: 'Office-Termine in Google Calendar synchronisieren',
      enabled: 'Google Calendar Sync aktivieren',
      credentialsOk:
        'Service Account und Impersonation sind unter Speicher & Cloud hinterlegt.',
      credentialsMissing:
        'Zuerst unter Speicher & Cloud Service Account JSON und Impersonation-E-Mail hinterlegen.',
      openStorage: 'Zu Speicher & Cloud',
      impersonate: 'Termine landen im Google-Kalender von',
      setupTitle: 'Google-Admin (einmalig)',
      setupCalendarApi:
        'In der Google Cloud Console die Google Calendar API für das Projekt aktivieren.',
      setupDwd:
        'In der Google Admin Console Domain-Wide Delegation: Scope https://www.googleapis.com/auth/calendar für denselben Service Account ergänzen (zusätzlich zu Drive-/Contacts-Scopes).',
      testConnection: 'Verbindung testen',
      testing: 'Wird getestet …',
      save: 'Speichern',
      saving: 'Wird gespeichert …',
      toast: {
        saved: 'Calendar-Konfiguration gespeichert.',
        testSuccess: 'Calendar API Verbindung erfolgreich.',
        testFailed: 'Verbindungstest fehlgeschlagen.',
        error: 'Aktion fehlgeschlagen.',
      },
    },
    nav: {
      email: 'E-Mail-Konfiguration',
      storage: 'Speicher & Cloud',
      contacts: 'Google Contacts',
      calendar: 'Google Calendar',
      ai: 'KI / Assistent',
      breakRules: 'Pausenregeln',
      activityTypes: 'Tätigkeitsbereiche',
      system: 'Server-Status',
      backup: 'Backup & Restore',
      features: 'Feature-Flags',
      general: 'Allgemein',
      billing: 'Verrechnung',
      products: 'Produkte',
      users: 'Benutzerverwaltung',
    },
    billing: {
      title: 'Verrechnung',
      subtitle: 'Nummernkreise, Zahlungsziele, Skonto und Leistungsort-MwSt',
      back: 'Zurück zu Einstellungen',
      readOnly: 'Nur SUPERADMIN kann speichern.',
      save: 'Speichern',
      saving: 'Wird gespeichert …',
      series: {
        title: 'Nummernkreise',
        reTitle: 'Ausgangsrechnungen (RE)',
        stTitle: 'Stornorechnungen (ST)',
        koTitle: 'Rechnungskorrekturen (KO)',
        prefix: 'Prefix',
        nextNumber: 'Nächste Nummer',
        preview: 'Vorschau',
        previewLabel: (v: string): string => `Nächste: ${v}`,
      },
      paymentTerms: {
        title: 'Zahlungsziele',
        defaultDays: 'Standard-Zahlungsziel (Tage)',
        options: 'Vorlagen (Tage, kommagetrennt)',
        optionsHint: 'z. B. 7, 14, 30, 60',
      },
      skonto: {
        title: 'Skonto',
        percent: 'Skonto (%)',
        days: 'Skonto-Frist (Tage)',
        template: 'PDF-Hinweistext',
        templateHint:
          'Platzhalter: {{skontoPercent}}, {{skontoDays}}, {{skontoAmount}}, {{dueDate}}, {{invoiceNumber}}, {{companyName}}',
      },
      reverseCharge: {
        title: 'Reverse Charge',
        pdfText: 'PDF-Hinweistext (§13b)',
        pdfTextHint:
          'Wird auf Rechnungen mit Steuerart Reverse Charge ausgegeben.',
      },
      countries: {
        title: 'Leistungsort / MwSt',
        code: 'Land-Code',
        name: 'Name',
        standardRate: 'Regelsteuersatz (%)',
        reducedRate: 'Ermäßigt (%)',
        add: 'Land hinzufügen',
        remove: 'Entfernen',
      },
      toast: {
        saved: 'Verrechnung-Einstellungen gespeichert.',
        error: 'Speichern fehlgeschlagen.',
      },
    },
    products: {
      title: 'Produkte',
      subtitle: 'Produktkatalog für Rechnungspositionen',
      back: 'Zurück zu Einstellungen',
      add: 'Produkt anlegen',
      edit: 'Bearbeiten',
      deactivate: 'Deaktivieren',
      activate: 'Aktivieren',
      empty: 'Noch keine Produkte vorhanden.',
      code: 'Code',
      name: 'Name',
      unit: 'Einheit',
      defaultUnitPrice: 'Standardpreis (€)',
      defaultTaxRate: 'Standard-MwSt (%)',
      active: 'Aktiv',
      save: 'Speichern',
      saving: 'Wird gespeichert …',
      cancel: 'Abbrechen',
      toast: {
        saved: 'Produkt gespeichert.',
        deleted: 'Produkt entfernt bzw. deaktiviert.',
        error: 'Aktion fehlgeschlagen.',
      },
    },
    ai: {
      title: 'KI / Assistent',
      subtitle: 'OpenAI-kompatible API für Kontakt-Import und Assistent',
      enabled: 'KI-Assistent aktivieren',
      baseUrl: 'Base-URL',
      baseUrlPlaceholder: 'https://api.openai.com/v1',
      model: 'Modell',
      modelPlaceholder: 'gpt-4.1-mini',
      apiKey: 'API-Key',
      apiKeyPlaceholder: 'sk-… (leer lassen = unverändert)',
      apiKeyConfigured: 'API-Key hinterlegt',
      apiKeyMissing: 'Kein API-Key hinterlegt',
      timeoutMs: 'Timeout (ms)',
      testConnection: 'Verbindung testen',
      testing: 'Wird getestet …',
      save: 'Speichern',
      saving: 'Wird gespeichert …',
      info: 'Der Key wird nur serverseitig gespeichert und in der UI maskiert angezeigt.',
      toast: {
        saved: 'KI-Konfiguration gespeichert.',
        testSuccess: 'KI-Verbindung erfolgreich.',
        testFailed: 'Verbindungstest fehlgeschlagen.',
        error: 'Aktion fehlgeschlagen.',
      },
    },
    general: {
      title: 'Allgemeine Einstellungen',
      subtitle: 'Schalter für Kiosk und Betrieb',
      back: 'Zurück zu Einstellungen',
      readOnly: 'Nur SUPERADMIN oder OFFICE können speichern.',
      kioskDebugTitle: 'Kiosk-Debug-Log',
      kioskDebugHint:
        'Wenn aktiv, zeigt das Stempel-Terminal unten rechts ein Log (API-Aufrufe, Navigation). Schreiben erfolgt weiterhin nur bei aktivem Schalter.',
      gpsIntervalTitle: 'GPS-Speicherintervall',
      gpsIntervalHint:
        'Während einer aktiven Schicht (Monteur-App) wird der Standort in diesem Abstand zusätzlich gespeichert. Ein- und Ausstempeln speichern immer einen Punkt. Standard: 20 Minuten.',
      gpsIntervalUnit: 'Minuten',
      pinLengthTitle: 'PIN-Länge',
      pinLengthHint:
        'Anzahl der Ziffern für Stempeluhr-, Kiosk- und Kunden-PL-PINs (4–8). Nach Änderung müssen bestehende PINs neu gesetzt werden.',
      pinLengthUnit: 'Ziffern',
      overtimeAlertTitle: 'Arbeitszeit-Alarm',
      overtimeAlertHint:
        'Wenn ein Monteur durchgehend länger als die eingestellte Stundenzahl eingestempelt ist, geht eine kurze E-Mail an die Empfänger-Adresse (Name, Projekt, Dauer). Ohne E-Mail = Alarm aus. SMTP muss unter E-Mail konfiguriert sein.',
      overtimeAlertEmailLabel: 'Empfänger-E-Mail',
      overtimeAlertEmailPlaceholder: 'leitung@firma.de',
      overtimeAlertHoursLabel: 'Alarm nach Stunden',
      overtimeAlertHoursHint: 'Durchgehend eingestempelt (1–24). Standard: 10.',
      overtimeAlertHoursUnit: 'Stunden',
      overtimeAlertRemindersLabel: 'Anzahl Erinnerungen',
      overtimeAlertRemindersHint:
        'Wie oft insgesamt gemeldet wird (inkl. erster Alarm), solange die Stempelung offen ist (1–10).',
      overtimeAlertReminderIntervalLabel: 'Abstand',
      overtimeAlertReminderIntervalUnit: 'Minuten',
      overtimeAlertReminderIntervalHint:
        'Minuten zwischen zwei Meldungen derselben Stempelung (5–240). Cron prüft alle 5 Minuten.',
      overtimeAlertOnceHint:
        'Nach Speichern der Alarm-Einstellungen darf erneut gemeldet werden. Cron prüft alle 5 Minuten.',
      overtimeAlertTestButton: 'Test-Mail senden',
      overtimeAlertRunButton: 'Jetzt prüfen',
      overtimeAlertTesting: 'Test-Mail wird gesendet …',
      overtimeAlertRunning: 'Wird geprüft …',
      autoClockOutTitle: 'Auto-Ausstempelung',
      autoClockOutHint:
        'Wenn aktiv, werden offene Schichten nach der eingestellten Stundenzahl systemseitig beendet. Der Ausstempel-Zeitpunkt ist Einstempeln plus Schwelle (nicht „jetzt“). Alarm und Auto-Out sind getrennt – der Alarm sollte typischerweise früher greifen.',
      autoClockOutHoursLabel: 'Nach wie vielen Stunden automatisch ausstempeln?',
      autoClockOutHoursHint: 'Durchgehend eingestempelt (1–24). Standard: 12.',
      autoClockOutHoursUnit: 'Stunden',
      autoClockOutEmailHint:
        'Optional: Benachrichtigung an dieselbe Empfänger-E-Mail wie beim Arbeitszeit-Alarm.',
      autoClockOutRunButton: 'Jetzt prüfen',
      autoClockOutRunning: 'Wird geprüft …',
      noShowAlertTitle: 'Erinnerung: nicht eingestempelt',
      noShowAlertHint:
        'Wenn aktiv, prüft das System Mo–Fr ab der eingestellten Uhrzeit (Europe/Berlin), ob zugewiesene Monteure auf aktiven Projekten noch keinen Clock-In haben. Krank / Urlaub / nicht verfügbar werden ausgenommen.',
      noShowAlertHourLabel: 'Stunde',
      noShowAlertMinuteLabel: 'Minute',
      noShowAlertTimeHint:
        'Check-Zeit Europe/Berlin (Mo–Fr). Standard: 10:00.',
      noShowAlertEmailHint:
        'Empfänger ist dieselbe E-Mail wie beim Arbeitszeit-Alarm. Ohne gültige Adresse kein Versand.',
      noShowAlertRunButton: 'Jetzt prüfen',
      noShowAlertRunning: 'Wird geprüft …',
      documentUploadTitle: 'Max. Upload-Größe (MB)',
      documentUploadHint:
        'Obergrenze für Pläne und Dokumente (5–64 MB). Standard: 50. Max. 64 MB wegen nginx (client_max_body_size 64m).',
      documentUploadUnit: 'MB',
      on: 'An',
      off: 'Aus',
      save: 'Speichern',
      saving: 'Wird gespeichert …',
      toast: {
        saved: 'Einstellungen gespeichert.',
        overtimeTestSent: 'Test-Mail wurde gesendet.',
        overtimeTestFailed: 'Test-Mail fehlgeschlagen.',
        overtimeRunDone: 'Prüfung abgeschlossen.',
        overtimeRunNone: 'Keine überschrittene Stempelung gefunden.',
        autoClockOutDone: 'Auto-Ausstempelung abgeschlossen.',
        autoClockOutNone: 'Keine Schicht geschlossen.',
        autoClockOutDisabled: 'Auto-Ausstempelung ist deaktiviert.',
        noShowAlertDone: 'No-Show-Prüfung abgeschlossen.',
        noShowAlertNone: 'Alle zugewiesenen Monteure haben eingestempelt.',
        noShowAlertDisabled: 'No-Show-Erinnerung ist deaktiviert – kein Versand.',
        noShowAlertWeekend: 'Wochenende: manuelle Prüfung trotzdem ausgeführt.',
        error: 'Speichern fehlgeschlagen.',
      },
    },
    features: {
      title: 'Feature-Flags',
      subtitle: 'Kernmodule ein- oder ausblenden (Navigation und API)',
      back: 'Zurück zu Einstellungen',
      save: 'Speichern',
      saving: 'Wird gespeichert …',
      readOnlyHint: 'Nur Superadmins können Feature-Flags ändern.',
      toast: {
        saved: 'Feature-Flags gespeichert.',
        error: 'Speichern fehlgeschlagen.',
      },
    },
    users: {
      title: 'Benutzerverwaltung',
      subtitle: 'Benutzer, Rollen und Berechtigungen',
      back: 'Zurück zu Einstellungen',
      tabUsers: 'Benutzer',
      tabRoles: 'Rollen & Rechte',
      add: 'Benutzer anlegen',
      edit: 'Bearbeiten',
      deactivate: 'Deaktivieren',
      activate: 'Aktivieren',
      empty: 'Noch keine Benutzer vorhanden.',
      columns: {
        name: 'Name',
        email: 'E-Mail',
        roles: 'Rollen',
        status: 'Status',
        actions: 'Aktionen',
      },
      active: 'Aktiv',
      inactive: 'Inaktiv',
      form: {
        displayName: 'Anzeigename',
        email: 'E-Mail',
        password: 'Passwort',
        passwordOptional: 'Neues Passwort (optional)',
        roles: 'Rollen',
        notes: 'Notizen',
        save: 'Speichern',
        saving: 'Wird gespeichert …',
        cancel: 'Abbrechen',
        createTitle: 'Benutzer anlegen',
        editTitle: 'Benutzer bearbeiten',
      },
      matrix: {
        title: 'Berechtigungsmatrix',
        hint: 'Häkchen setzen und pro Rolle speichern. SUPERADMIN hat immer Vollzugriff.',
        permission: 'Berechtigung',
        saveRole: 'Rolle speichern',
        saving: 'Wird gespeichert …',
      },
      toast: {
        created: 'Benutzer angelegt.',
        saved: 'Benutzer gespeichert.',
        deactivated: 'Benutzer deaktiviert.',
        roleSaved: 'Rollen-Berechtigungen gespeichert.',
        error: 'Aktion fehlgeschlagen.',
        noPermission: 'Keine Berechtigung für die Benutzerverwaltung.',
      },
    },
    backup: {
      title: 'Backup & Restore',
      subtitle: 'Datenbank-Backups planen, manuell erstellen und Module wiederherstellen',
      scheduleTitle: 'Zeitplan',
      enabled: 'Automatisches Backup aktivieren',
      hour: 'Stunde (0–23, Europe/Berlin)',
      minute: 'Minute (0–59, Europe/Berlin)',
      retentionDays: 'Aufbewahrung (Tage)',
      scheduleHint:
        'Die Uhrzeit gilt für Europe/Berlin (deutsche Ortszeit), nicht UTC.',
      save: 'Speichern',
      saving: 'Wird gespeichert …',
      lastUpdated: 'Zuletzt geändert',
      startBackup: 'Jetzt sichern',
      running: 'Backup läuft …',
      jobsTitle: 'Vorhandene Backups',
      restoresTitle: 'Restore-Protokoll',
      noJobs: 'Noch keine Backups vorhanden.',
      restore: 'Wiederherstellen',
      restoring: 'Wird wiederhergestellt …',
      restoreTitle: 'Backup wiederherstellen',
      restoreConfirm:
        'Gewählte Module werden aus dem Backup in die aktuelle Datenbank zurückgeschrieben (Upsert). Bitte Module sorgfältig wählen.',
      deleteTitle: 'Backup löschen?',
      deleteConfirm: 'Backup-Dateien und Job-Eintrag werden unwiderruflich gelöscht.',
      triggerCron: 'Zeitplan',
      triggerManual: 'Manuell',
      columns: {
        created: 'Erstellt',
        status: 'Status',
        trigger: 'Auslöser',
        size: 'Größe',
        actions: 'Aktionen',
        modules: 'Module',
      },
      toast: {
        saved: 'Backup-Konfiguration gespeichert.',
        backupOk: 'Backup erfolgreich erstellt.',
        backupFailed: 'Backup fehlgeschlagen.',
        restoreOk: 'Wiederherstellung erfolgreich.',
        restorePartial: 'Wiederherstellung teilweise erfolgreich.',
        restoreFailed: 'Wiederherstellung fehlgeschlagen.',
        deleted: 'Backup gelöscht.',
        error: 'Aktion fehlgeschlagen.',
      },
    },
    system: {
      title: 'Server-Status',
      subtitle: 'Systemüberwachung und Ressourcenauslastung',
      refresh: 'Aktualisieren',
      lastUpdated: 'Aktualisiert',
      autoRefresh: 'Daten werden alle 30 Sekunden aktualisiert.',
      loading: 'Lade Systemdaten …',
      error: 'Systemdaten konnten nicht geladen werden.',
      retry: 'Erneut versuchen',
      backToSettings: 'Einstellungen',
      summary: {
        customers: 'Kunden',
        projects: 'Projekte',
        workers: 'Monteure',
        openTasks: 'Offene Aufgaben',
        uptime: 'Uptime',
      },
      cpu: {
        title: 'CPU',
        model: 'Modell',
        cores: 'Kerne',
        usage: 'Auslastung',
        loadAvg: 'Load Average',
      },
      memory: {
        title: 'Arbeitsspeicher',
        total: 'Gesamt',
        used: 'Belegt',
        free: 'Frei',
      },
      disk: {
        title: 'Festplatte',
        total: 'Gesamt',
        used: 'Belegt',
        available: 'Verfügbar',
      },
      services: {
        title: 'Service Health Checks',
        online: 'Online',
        offline: 'Offline',
        responseTime: 'ms',
      },
      updates: {
        title: 'OS Updates',
        containerUpdates: 'Container-Updates',
        hostUpdates: 'Host-Updates',
        noUpdates: 'Keine Updates verfügbar',
        hostNotAvailable: 'Host-Updates nicht verfügbar (kein Host-Zugriff)',
        packagesAvailable: (n: number): string =>
          n === 1 ? '1 Paket verfügbar' : `${n} Pakete verfügbar`,
        installButton: 'Updates installieren',
        installing: 'Wird installiert …',
        confirmTitle: 'Updates installieren?',
        confirmText:
          'Alle verfügbaren Container-Pakete werden aktualisiert. Fortfahren?',
        confirmButton: 'Installieren',
        cancelButton: 'Abbrechen',
        success: 'Updates erfolgreich installiert.',
        failed: 'Update-Installation fehlgeschlagen.',
      },
      database: {
        title: 'PostgreSQL Datenbank',
        size: 'Datenbankgröße',
        connections: 'Verbindungen',
        activeOf: 'aktiv von',
        cacheHitRatio: 'Cache Hit Ratio',
        version: 'Version',
        topTables: 'Top 15 Tabellen',
        tableName: 'Tabelle',
        tableSize: 'Größe',
        tableRows: 'Zeilen',
      },
      storage: {
        title: 'MinIO Storage',
        totalSize: 'Gesamtgröße',
        totalObjects: 'Objekte gesamt',
        bucketName: 'Bucket',
        bucketObjects: 'Objekte',
        bucketSize: 'Größe',
        unavailable: 'MinIO nicht erreichbar',
      },
      network: {
        title: 'Netzwerk',
        interface: 'Schnittstelle',
        received: 'Empfangen',
        sent: 'Gesendet',
      },
      serverDetails: {
        title: 'Server-Details',
        hostname: 'Hostname',
        platform: 'Plattform',
        arch: 'Architektur',
        nodeVersion: 'Node.js',
      },
      processes: {
        title: 'Top 10 Prozesse',
        pid: 'PID',
        user: 'Benutzer',
        cpu: 'CPU %',
        mem: 'RAM %',
        command: 'Befehl',
      },
      appStats: {
        title: 'Anwendungsstatistiken',
        communication: 'Kommunikation (7 Tage)',
        documents: 'Dokumente gesamt',
        equipmentAssigned: 'Geräte zugewiesen',
        equipmentAvailable: 'Geräte verfügbar',
      },
      osUsers: {
        title: 'OS-Benutzer',
      },
    },
  } as const;
