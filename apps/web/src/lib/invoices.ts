...
export async function downloadInvoicePdf(
  ...
  await downloadAuthenticatedFile(invoicesApi.pdfUrl(id), filename, 'PDF');
}

export async function downloadInvoiceXRechnung(
  id: string,
  filename: string,
): Promise<void> {
  await downloadAuthenticatedFile(
    invoicesApi.xrechnungUrl(id),
    filename,
    'XRechnung',
  );
}

export async function downloadInvoiceZugferd(
  id: string,
  filename: string,
): Promise<void> {
  await downloadAuthenticatedFile(
    invoicesApi.zugferdUrl(id),
    filename,
    'ZUGFeRD',
  );
}
...
async function downloadAuthenticatedFile(
  ...
  if (!res.ok) {
    throw new ApiError(
      `${label}-Export fehlgeschlagen (${res.status})`,
      res.status,
    );
  }
